...
// Get concerts created and attended by user
export async function getAllConcerts(username) {
  try {
...
export async function getArtistConcerts(username) {
  try {
    const concerts = await databases.listDocuments(
      config.databaseId,
      config.concertsCollectionId,
      [Query.equal("artist", [username])]
    );
...
export async function getAudienceConcerts(username) {
  try {
    const concerts = await databases.listDocuments(
      config.databaseId,
      config.concertsCollectionId,
      [Query.contains("audience", [username])]
    );
...
// Get a concert
export async function getConcert(concert) {
  try {
    const concerts = await databases.listDocuments(
      config.databaseId,
      config.concertsCollectionId,
      [Query.equal("$id", [concert.$id])]
    );

    return concerts.documents[0];
  } catch (error) {
    throw new Error(error);
  }
}